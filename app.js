document.addEventListener('DOMContentLoaded', () => {
  ...
  //add an onclick to each square in your grid
      squares[i].onclick = function(){
          ...
          if(squares[index + 7].classList.contains('taken')){
              if (currentPlayer === 1) {
                ...
                if (!squares[index].hasClass('player-one')) {
                  squares[index].classList.add('taken')
      ...
      squares[i].addEventListener("mouseover", function(e) {
        if (squares[index + 7].classList.contains('taken')){
          squares[index].classList.add('validNextMove');
        }
      })
      
      squares[i].addEventListener("mouseleave", function(e) {
        if (squares[index + 7].classList.contains('taken')){
          squares[index].classList.remove('validNextMove');
        }
      })
